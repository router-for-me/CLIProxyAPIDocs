...
- `SUBSCRIBE usage` 使用 Redis pub/sub 响应格式。只要至少有一个客户端订阅，新的 usage 记录会广播给所有已订阅客户端，并且不会写入 FIFO 队列；这些记录之后无法再通过 `LPOP`/`RPOP` 或 Management usage 队列接口查询到。
- 如果当前没有客户端订阅，新的 usage 记录仍会按原逻辑进入 FIFO 队列。
- 订阅模式下支持 `PING`、`UNSUBSCRIBE`、`QUIT` 做基础连接控制。
