...
| Входная возможность | `model_router` | [Маршрутизатор моделей](./model-router) |
...
| Ресурсная страница плагина | `resources` | `/v0/resource/plugins/<pluginID>/...` | Сам resource request не проходит management-аутентификацию. В same-origin deployment Management Center доверенный JavaScript страницы может прочитать сохранённый management key и вызывать `/v0/management/...`. |
...
- Установка и включение плагина с resource pages означает доверие браузерному коду этого плагина. В same-origin deployment этот код может читать `localStorage` Management Center, включая сохранённый management key, если он есть.
- Чувствительные операции держите за маршрутами `/v0/management/...`. Resource page должна вызывать эти маршруты с сохранённым management key, а не выполнять чувствительную работу напрямую из неаутентифицированного resource GET-запроса.
- Скрипты resource page должны поставляться вместе с плагином. Не загружайте сторонние скрипты на страницу, которая может получить доступ к same-origin management-контексту.
...
- Не раскрывайте host callbacks, которые читают учётные данные, записывают учётные данные или выполняют привилегированные действия, напрямую через неаутентифицированные query parameters ресурса. Если пользовательский UI должен запускать такие действия, используйте same-origin доверенную resource page и аутентифицированный вызов `/v0/management/...`.
